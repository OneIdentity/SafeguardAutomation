; NEWS SINCE LAST RELEASE (3.5)
; - Enhanced debugging
; -- if debug=1 the WD console remains open on the screen
; -- few more logs
; - use wd.conf file from webdriver folder
; - modified wd_core.au3, $skipWDVersionCheck -- allows skipping _WD_IsLatestRelease connect to Internet
...
;
; The code disables user input and hides the WebDriver console when not running in debug mode

$web_generic_script = @ScriptName
...
EndIf
Local $loglevel = ''
Local $wd_conf_file = @ScriptDir & '\wd.conf'
...
Logger('info', "Starting " & $web_generic_script & " with loglevel:" & $loglevel)
Logger('debug', "browser=" & $browser)

; Read webdriver configuration from webdriver\wd.conf file
Logger('debug', "Reading wd configuration from: " & $wd_conf_file)
Local $hFileOpen = FileOpen($wd_conf_file, $FO_READ)
If $hFileOpen = -1 Then
    Logger('info', "An error occurred when reading the wd conf file: " & $wd_conf_file)
    Exit
EndIf
...
While 1
	...
	; skip comments with parsing
	If StringLeft($sFileRead, 1) <> "#" Then
		Logger('debug', "wd.conf: " & $sFileRead)
		Local $wd_conf = StringSplit ($sFileRead, "=")
		Switch $wd_conf[1]
			Case "skipWDVersionCheck"
				$skipWDVersionCheck=$wd_conf[2]
			Case "chromeCapabilities"
				$chromeCapabilities=$wd_conf[2]
			Case "firefoxCapabilities"
				$firefoxCapabilities=$wd_conf[2]
			Case "edgeCapabilities"
				$edgeCapabilities=$wd_conf[2]
			Case "WDConsoleVisible"
				$WDConsoleVisible=$wd_conf[2]
			Case Else
				Logger('info', "Unknown webdriver configuration in wd.conf: " & $wd_conf[1])
				Exit
		EndSwitch
	EndIf
WEnd
...
Switch $browser
	Case 'chrome'
		SetupChrome()
		$sDesiredCapabilities = $chromeCapabilities
	Case 'c'
		SetupChrome()
		$sDesiredCapabilities = $chromeCapabilities
	Case 'firefox'
		SetupGecko()
		$sDesiredCapabilities = $firefoxCapabilities
	Case 'f'
		SetupGecko()
		$sDesiredCapabilities = $firefoxCapabilities
	Case 'edge'
		SetupEdge()
		$sDesiredCapabilities = $edgeCapabilities
	Case 'e'
		SetupEdge()
		$sDesiredCapabilities = $edgeCapabilities
EndSwitch
...
Else
	Logger('debug', 'User input is not disabled in debug mode')
EndIf

;_WD_Startup() in wd_core.au3 connects to the Internet for checking
Logger('debug', 'Starting webdriver with skipWDVersionCheck=' & $skipWDVersionCheck)
_WD_Startup($skipWDVersionCheck)
...
If $WDConsoleVisible = 0 Then
	_WD_ConsoleVisible(false)
Else
	Logger('debug', 'WDConsoleVisible=1, Keeping WebDriver console visible')
EndIf
...
Func Logger($level, $msg)
	...
	if $level == "debug" And $debug = 1 Then
		FileWriteLine($logfile, $ts & " -- DEBUG -- " & $msg)
	ElseIf $level == "info" Then
		FileWriteLine($logfile, $ts & " -- INFO -- " & $msg)
	EndIf
EndFunc
...
Func SetupGecko()
	...
	;$sDesiredCapabilities = '{"capabilities": {"alwaysMatch": {"moz:firefoxOptions":{"args": ["-kiosk"], "prefs": {' & $ff_preferences & '}},"browserName": "firefox", "acceptInsecureCerts":true}}}'
...
Func SetupChrome()
   ...
   ; Add chromeOption to not offer saving credentials
	...
	;if $mode=="kiosk" Then
	;	$sDesiredCapabilities = '{"capabilities": {"alwaysMatch": {"goog:chromeOptions": {"args": ["--kiosk"], "w3c": true, "excludeSwitches": [ "enable-automation"], "prefs": { "credentials_enable_service": false, "profile": { "password_manager_enabled": false}}} }}}'
	;ElseIf $mode == "normal" Then
